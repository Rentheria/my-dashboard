...
export const PokemonDetail = ({ pokemon }: Props) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-900 text-slate-300 font-mono pb-10">
      {/* Hero Section */}
      <div
        className={`relative w-full h-75 md:h-100 bg-linear-to-b ${colors.gradient} overflow-hidden border-b-4 ${colors.border}`}
      >
        <div className="absolute top-4 left-4 z-50">
          <Link
            href="/dashboard/pokemons"
            className="flex items-center gap-2 px-4 py-2 bg-black/30 hover:bg-black/50 text-white rounded-full backdrop-blur-md transition-colors border border-white/20 hover:border-white/50 group"
          >
        ...
        <div className="absolute inset-0 bg-[url('https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png')] opacity-10 bg-repeat space-x-4 pixelated"></div>
        <div className="absolute inset-x-0 bottom-0 h-32 bg-linear-to-t from-slate-900 to-transparent"></div>

        <div className="relative z-10 flex flex-col items-center justify-center h-full gap-4">
          <h1
            className={`text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-linear-to-r ${colors.gradient} drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)] uppercase tracking-widest pixelated-text`}
            style={{
      ...
      <div className="container mx-auto px-4 -mt-20 relative z-20">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-6">
            <div
              className={`bg-slate-800/80 backdrop-blur-xl rounded-3xl p-8 border ${colors.border} shadow-2xl shadow-black/50 relative overflow-hidden group`}
            >
              <div
                className={`absolute top-0 right-0 p-4 opacity-20 group-hover:opacity-40 transition-opacity ${colors.text}`}
              >
                <svg
                  className="w-32 h-32"
                  fill="currentColor"
              ...
              <div className="relative z-10 flex justify-center py-6">
                <Image
                  ...
                  className="drop-shadow-[0_20px_20px_rgba(0,0,0,0.5)] transform group-hover:scale-110 transition-transform duration-500 ease-in-out pixelated"
                  priority
                />
              </div>
              ...
              <div className="flex justify-center gap-4 mt-6">
                {pokemon.types.map((type) => {
                  ...
                  return (
                    <span
                      key={type.slot}
                      className={`px-6 py-2 rounded-sm ${typeColor.bg} border-2 ${typeColor.border} text-white font-bold uppercase tracking-wider hover:opacity-90 transition-all cursor-default shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,0.3)] hover:translate-x-0.5 hover:translate-y-0.5`}
            ...
            <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-6 border border-white/5">
              <h3
                className={`text-lg font-bold ${colors.text} mb-4 uppercase tracking-wider flex items-center gap-2`}
              >
              ...
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  .map((sprite, idx) => (
                    <div
                      key={idx}
                      className={`bg-slate-900/50 rounded-xl p-2 border border-white/5 hover:${colors.border} transition-colors flex justify-center items-center`}
                    >
                      <Image
                        src={sprite!}
                        width={80}
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-6">
            ...
            <div className="bg-slate-800/60 backdrop-blur-md rounded-3xl p-8 border border-white/5 grow">
              <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-3">
                <span
                  className={`p-2 ${colors.accent} rounded-lg ${colors.text}`}
                >
                  <svg
                    className="w-6 h-6"
                    fill="none"
        ...
        <div className="flex justify-between items-center mt-8 pb-10">
          <Link
            href={`/dashboard/pokemon/${prevId}`}
            className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl border border-white/10 hover:border-white/30 transition-all group"
          >
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="font-bold">#{prevId}</span>
            </div>
          </Link>

          <Link
            href={`/dashboard/pokemon/${nextId}`}
            className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl border border-white/10 hover:border-white/30 transition-all group"
          >
            <div className="flex flex-col items-end">
              ...
              <span className="font-bold">#{nextId}</span>
            </div>
